...
static reloc_howto_type elf_nios2_howto_table_rel[] = {
 ...
 /* Not valid for Nios II, included for completeness */
      ...
      if (ELF32_R_TYPE (irel->r_info) == (int) R_NIOS2_CJMP)
	{
	  ...
	  if ((irelalign == NULL && (long) pcrel_offset <= 0x8008
		...
		case OP_MATCH_BNE:
		  ...
		  break;
		case OP_MATCH_BGE:
		  opcode = (opcode & ~OP_MASK_OP) | OP_MATCH_BLT;
		  break;
		case OP_MATCH_BGEU:
		  opcode = (opcode & ~OP_MASK_OP) | OP_MATCH_BLTU;
		  break;
		case OP_MATCH_BLT:
		  opcode = (opcode & ~OP_MASK_OP) | OP_MATCH_BGE;
		  break;
		case OP_MATCH_BLTU:
		  opcode = (opcode & ~OP_MASK_OP) | OP_MATCH_BGEU;
		  break;
		default:
		  fprintf (stderr,
