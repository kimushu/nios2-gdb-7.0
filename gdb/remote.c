...
/**
	  ALTERA FIX: Eclipse can't deal with remote commands responding
	  on the target stream that outbuf represents. Instead, Eclipse
	  expects the response to come throught the stdout stream. Here
	  we change to stdout instead of the target stream. More info on
	  SPR:357900. --Silvio/10 
*/
	  if(gdb_stdout)
	  {
