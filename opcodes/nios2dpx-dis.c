...
/* Maximum length for a register name */
#define REG_STR_MAX 16
...
/* ToDo: how to get this value in a data driven way? */
#define NIOS2_HASH_SIZE 1024
static nios2_opcode_hash *nios2_hash[NIOS2_HASH_SIZE];

/* separate hash table for pseudo-ops */
/* ToDo: Size here doesn't make sense to me */
static nios2_opcode_hash *nios2_ps_hash[NIOS2_HASH_SIZE];
  ...
  for (i = 0; i < NIOS2_HASH_SIZE; i++)
      ...
      for (op = nios2_opcodes; op < &nios2_opcodes[NUMOPCODES]; op++)
	{
	  if ((op->pinfo & NIOS2_INSN_MACRO) == NIOS2_INSN_MACRO)
	    {
	      if (i == (((unsigned)op->match >> OP_SH_OP) & OP_MASK_OP) &&
		  (op->
	  ...
	  else if (i == (((unsigned)op->match >> OP_SH_OP) & OP_MASK_OP))
	    {
	  ...
	  if (argstr != NULL && *argstr != '\0')
	    {
	      ...
	      while (*argstr != '\0')
		{
		  nios2_print_insn_arg (argstr, opcode, address, 
					info, (*argstr == op->dest_reg));
		  ++argstr;
		}
	    }
	}
...
nios2_print_insn_arg (const char *argptr,
		      ...
		      disassemble_info * info, bfd_boolean is_dest)
{
  unsigned int i = 0;
  int nios2_extension_register_cfg = 0;

  assert (argptr != NULL);
  assert (info != NULL);
  if (info->section != NULL) {
    assert (info->section->owner != NULL);
    assert (elf_elfheader (info->section->owner) != NULL);

    unsigned int e_flags = elf_elfheader (info->section->owner)->e_flags;
    nios2_extension_register_cfg = E_NIOS2_REG_MAP_GET(e_flags);
  }

  assert (nios2_extension_register_cfg >= 0 && nios2_extension_register_cfg < nios2_num_ext_reg_maps);
  ...
  switch (*argptr)
    ...
    case 'd':
      ...
      if (((opcode & OP_MASK_CUSTOM) == OP_MATCH_CUSTOM))
        {
          /* custom instruction registers are always 5 bits */
          i &= 0x1f;
          if (GET_INSN_FIELD (CUSTOM_C, opcode) == 0)
            {         
              (*info->fprintf_func) (info->stream, "c%d", i);
              break;
            }
        }
      ...
      if (i < NUMREGNAMES)
        {
	  char alias[REG_STR_MAX];	 
	  if (nios2_get_register_alias(alias, REG_STR_MAX, nios2_regs[i].name, nios2_extension_register_cfg, is_dest) > 0)
	    (*info->fprintf_func) (info->stream, "%s", alias);
	  else
      ...
      if (((opcode & OP_MASK_CUSTOM) == OP_MATCH_CUSTOM))
        {
          ...
          if (GET_INSN_FIELD (CUSTOM_A, opcode) == 0)
            {         
              (*info->fprintf_func) (info->stream, "c%d", i);
              break;
            }
      ...
      if (((opcode & OP_MASK_CUSTOM) == OP_MATCH_CUSTOM))
        {
          ...
          if (GET_INSN_FIELD (CUSTOM_B, opcode) == 0)
            {         
              (*info->fprintf_func) (info->stream, "c%d", i);
              break;
            }
    ...
    case 'n':
      ...
      i = (signed) (GET_INSN_FIELD (IMM12, opcode) << 20) >> 20;
      (*info->fprintf_func) (info->stream, "%d", i);
      break;
    case 'p':
      /* 12-bit unsigned immediate */
      i = GET_INSN_FIELD (IMM12, opcode);
      (*info->fprintf_func) (info->stream, "%d", i);
      break;
    case 'o':
    ...
    case 'h':
      ...
      i = GET_INSN_FIELD (BMX_MSB_IMM5, opcode);
      (*info->fprintf_func) (info->stream, "%d", i);
      break;
    case 'j':
      /* 5-bit unsigned immediate */
    ...
    case 'm':
      ...
      i = GET_INSN_FIELD (IMM26LO, opcode) | (GET_INSN_FIELD (IMM26HI, opcode) << OP_SH_IMM26LO_SIZE);
      /* this translates to an address because its only used in call instructions */
      address = (address & 0xf0000000) | (i << 2);
      (*info->print_address_func) (address, info);
      break;
    case 'c':
      i = GET_INSN_FIELD (RRT, opcode);	/* ctrl register index */
    ...
    case 'r':
      ...
      i = GET_INSN_FIELD (IMM20, opcode);
      (*info->fprintf_func) (info->stream, "%d", i);
      break;
    case 'q':
        /* 14-bit signed immediate right-shifted 2x */
    	/* copied from 'o' 16-bit signed immediate address offset */
      i =  (signed) (GET_INSN_FIELD (IMM12, opcode) << 20) >> 18;
      address = address + 4 + i;
      (*info->print_address_func) (address, info);
      break;
